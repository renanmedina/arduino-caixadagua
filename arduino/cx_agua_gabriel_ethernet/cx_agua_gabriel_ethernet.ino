...
void setup() {
  ...
  if(loadConfigs()){
    ...
    return;
    // initialize Ethernet using configs from SD File
    Ethernet.begin(confs.network.mac, confs.network.ip, confs.network.dns_srv, confs.network.gateway, confs.network.mask);  
  }
